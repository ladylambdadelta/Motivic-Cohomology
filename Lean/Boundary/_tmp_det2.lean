...
example (A : Type u) [CommRing A] :
  ...
  letI : Subsingleton PEmpty := inferInstance
  ...
  have hdet0 : LinearMap.det hQ.differential = (1 : A) := by
    simpa [hQ] using (LinearMap.det_eq_one_of_subsingleton (f := hQ.differential))
  let hQS : Algebra.SubmersivePresentation.{0, 0} A (MvPolynomial (Fin 1) A) := {
    toPreSubmersivePresentation := hQ,
    jacobian_isUnit := by
      rw [Algebra.PreSubmersivePresentation.jacobian]
      rw [hdet0]
    isFinite := by
      change P.IsFinite
