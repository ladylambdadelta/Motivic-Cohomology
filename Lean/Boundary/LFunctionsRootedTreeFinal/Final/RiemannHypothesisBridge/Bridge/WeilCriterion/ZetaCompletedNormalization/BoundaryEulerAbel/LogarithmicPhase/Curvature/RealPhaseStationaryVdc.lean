...
derivative is nonnegative on its shifted interval. -/
theorem Complex.logarithmicPhaseRealPhase_weylShift_deriv_nonneg_of_nonneg
    (t : ℝ)
    (ht : 1 ≤ ‖t‖)
    (ht_nonneg : 0 ≤ t)
    {a b : ℕ}
...
/-- On every canonical Weyl shift in the positive long branch, the shifted
derivative norm is the shifted derivative itself. -/
theorem Complex.logarithmicPhaseRealPhase_weylShift_deriv_norm_eq_of_nonneg
    (t : ℝ)
    (ht : 1 ≤ ‖t‖)
    (ht_nonneg : 0 ≤ t)
    {a b : ℕ}
