import Boundary.LFunctionsRootedTreeCanonicalAll._outside_RH_rooted_import_cone.Admissible.ZetaAdmissibleAutocorrelation.ZetaAdmissibleAutocorrelation
import Boundary.LFunctionsRootedTreeCanonicalAll._outside_RH_rooted_import_cone.Admissible.ZetaAdmissiblePaleyWiener.Admissible.ZetaAdmissibleProbe.Core.ZetaTransformCalculusBase.ZetaTransformCalculusBase
...
theorem admissibleProbeLaplaceFiniteSample_smul
  ...
  funext z
  unfold admissibleProbeLaplaceFiniteSample
  have hpoint :
...
theorem admissibleProbeLaplaceFiniteSampleCoordinate_self
  ...
  unfold admissibleProbeLaplaceFiniteSampleCoordinate
  exact if_pos rfl
...
theorem admissibleProbeLaplaceFiniteSample_dual_separating_ownerAdmissibleProbe
  ...
  apply LinearMap.ext
  intro a
  rcases admissibleProbeLaplaceFiniteSample_surjective_ownerAdmissibleProbe S a with
    ⟨f, hf⟩
  exact Eq.subst
...
theorem finiteExponentialDistribution_coefficients_eq_zero_of_tomography_ownerAdmissibleProbe
  ...
  have hΛvanish :
    ...
    intro f
    change finiteExponentialLaplacePairing S c f = 0
    exact hc f
  have hΛzero : Λ = 0 :=
