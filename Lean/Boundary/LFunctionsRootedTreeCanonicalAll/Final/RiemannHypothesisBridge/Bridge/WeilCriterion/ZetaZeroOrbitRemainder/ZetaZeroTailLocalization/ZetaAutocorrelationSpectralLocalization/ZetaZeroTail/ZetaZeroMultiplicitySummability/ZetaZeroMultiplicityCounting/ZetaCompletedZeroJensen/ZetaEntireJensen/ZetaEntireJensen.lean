...
theorem entireFunction_logOnCircle_bound_of_finiteOrder
  ...
  refine ⟨max 0 (Real.log A), ?_⟩
  intro R hR z hz_norm
  let E : ℝ := B * (1 + 2 * R) ^ m
  have hR_nonneg : 0 ≤ R :=
  ...
  · calc
      Real.log ‖F z‖ = Real.log 0 := by
        exact congrArg Real.log hzero
      _ = 0 := by
        exact Real.log_zero
      _ ≤ max 0 (Real.log A) + E := htarget_nonneg
  · have hpos : 0 < ‖F z‖ :=
      lt_of_le_of_ne (norm_nonneg (F z)) (Ne.symm hzero)
    have hfinite_z :
        ‖F z‖ ≤ A * Real.exp (B * (1 + ‖z‖) ^ m) :=
      hfinite z
    have hpow_arg :
