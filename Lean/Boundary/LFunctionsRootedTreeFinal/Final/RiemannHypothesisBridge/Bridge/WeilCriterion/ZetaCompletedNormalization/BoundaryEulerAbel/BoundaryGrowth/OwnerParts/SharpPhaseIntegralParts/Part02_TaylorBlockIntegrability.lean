...
theorem boundaryLineOnePointRealParam_firstPeriodicBernoulli_phaseIncrementLinear_integrable
  ...
  unfold boundaryLineOnePointRealParam_phaseIncrementLinearIntegrand
  let a : ℝ := ((((n - 1 : ℕ) : ℕ) : ℝ))
  let b : ℝ := (((n : ℕ) : ℝ))
  let c : ℂ :=
    (((-(t : ℂ) * Complex.I) / (a : ℂ)) *
...
theorem boundaryLineOnePointRealParam_firstPeriodicBernoulli_phaseIncrementRemainder_integrable
    ...
    Integrable
      ...
      (volume.restrict
        (Set.Ioc ((((n - 1 : ℕ) : ℕ) : ℝ)) (((n : ℕ) : ℝ)))) := by
  unfold boundaryLineOnePointRealParam_phaseIncrementRemainderIntegrand
  unfold boundaryLineOnePointRealParam_phaseIncrementLinearIntegrand
  let a : ℝ := ((((n - 1 : ℕ) : ℕ) : ℝ))
  let b : ℝ := (((n : ℕ) : ℝ))
  let c : ℂ :=
        ...
        B x *
          ((((x : ℝ) : ℂ) ^ (-(t : ℂ) * Complex.I)) -
            ((((((n - 1 : ℕ) : ℕ) : ℝ) : ℂ) ^
              (-(t : ℂ) * Complex.I))))) =
        (fun x : ℝ => B x * (L x + R x)) :=
    congrArg (fun phase : ℝ → ℂ => fun x : ℝ => B x * phase x) hphase
  have hcombined :
      (∫ x in s,
        B x *
          ((((x : ℝ) : ℂ) ^ (-(t : ℂ) * Complex.I)) -
            ((((((n - 1 : ℕ) : ℕ) : ℝ) : ℂ) ^
              (-(t : ℂ) * Complex.I))))) =
        ∫ x in s, B x * (L x + R x) :=
    congrArg (fun phase : ℝ → ℂ => ∫ x in s, phase x) hweighted
  have hpoint :
      (fun x : ℝ => B x * (L x + R x)) =
