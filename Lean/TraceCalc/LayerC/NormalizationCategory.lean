...
/-! ### Phase 8: exact truncation data from canonical threshold cuts -/

structure NormalizationTruncationData (obj : AdmNormObj) where
  threshold : Nat
...
namespace NormalizationTruncationData

def ofThreshold (obj : AdmNormObj) (threshold : Nat) (h : threshold ≤ obj.length) :
  NormalizationTruncationData obj where
  threshold := threshold
...
theorem lower_length_eq {obj : AdmNormObj} (truncation : NormalizationTruncationData obj) :
    (obj.lowerObj truncation.threshold truncation.threshold_le_length).length = truncation.threshold :=
  rfl

theorem upper_length_eq {obj : AdmNormObj} (truncation : NormalizationTruncationData obj) :
...
structure ExactTruncationData (obj : AdmNormObj) where
  truncation : NormalizationTruncationData obj
...
end NormalizationTruncationData
...
structure NormalizationPacketCutThread (obj : AdmNormObj) where
  ...
  threshold_le_length : threshold ≤ obj.length
  truncation : NormalizationTruncationData obj :=
    NormalizationTruncationData.ofThreshold obj threshold threshold_le_length
  exactTruncation : NormalizationTruncationData.ExactTruncationData obj :=
    NormalizationTruncationData.exactDataOfThreshold obj threshold threshold_le_length
