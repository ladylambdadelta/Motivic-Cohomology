...
theorem mk_directSum_eq_add (A B : EndomorphismObject.{u, v} K) :
  ...
  have h0 : mk K (directSumRelation K A B) = 0 := mk_directSumRelation (K := K) A B
  rw [directSumRelation_eq_sub] at h0
  exact h0
...
theorem determinantCharacter_directSumRelation (A B : EndomorphismObject.{u, v} K) :
  ...
  calc
    ...
    _ = 0 := by
          rw [EndomorphismObject.determinantUnit_product]
...
theorem determinantCharacter_conjRelation
  ...
  calc
    ...
    _ = 0 := by
          rw [EndomorphismObject.determinantUnit_conj A e]
          exact sub_eq_zero.mpr rfl
...
theorem determinantCharacter_zeroObject (A : EndomorphismObject.{u, v} K)
  ...
  rw [determinantCharacter_of, EndomorphismObject.determinantUnit_zeroObject (K := K) A hA]
  rfl
...
def determinantCharacterK0Mul : Multiplicative (K0 K) →* (RatFunc K)ˣ where
  ...
  map_one' := by
    ...
    exact rfl
  map_mul' x y := by
    have h := map_add (determinantCharacterK0 K) (Multiplicative.toAdd x) (Multiplicative.toAdd y)
    exact (congrArg Additive.toMul h).trans (toMul_add _ _)
...
theorem tracePower_zeroObject_zero (A : EndomorphismObject.{u, v} K)
  ...
  change LinearMap.trace K A.carrier (1 : A.carrier →ₗ[K] A.carrier) = 0
  have h : LinearMap.trace K A.carrier (1 : A.carrier →ₗ[K] A.carrier) =
      Module.finrank K A.carrier := by
    exact LinearMap.trace_one (R := K) (M := A.carrier)
  have hfin' : (Module.finrank K A.carrier : K) = 0 := by
    simpa using congrArg (fun m : ℕ => (m : K)) hfin
  exact h.trans hfin'
...
theorem tracePower_zeroObject_succ (n : ℕ) (A : EndomorphismObject.{u, v} K)
  ...
  haveI : Subsingleton A.carrier := hA
  have hpow : A.endomorphism ^ (n + 1) = 0 := by
    ext x
    exact Subsingleton.elim _ _
  change LinearMap.trace K A.carrier (A.endomorphism ^ (n + 1)) = 0
  have h : LinearMap.trace K A.carrier (A.endomorphism ^ (n + 1)) =
      LinearMap.trace K A.carrier (0 : A.carrier →ₗ[K] A.carrier) := by
    congr
  have hzero : LinearMap.trace K A.carrier (0 : A.carrier →ₗ[K] A.carrier) = 0 := by
    exact map_zero (LinearMap.trace K A.carrier)
  exact h.trans hzero
...
theorem powerMapVirtual_conjRelation
  ...
  calc
    ...
    _ = of K (EndomorphismObject.conj (EndomorphismObject.power A n) e) -
            of K (EndomorphismObject.power A n) := by
          have hconj :
              EndomorphismObject.conj (EndomorphismObject.power A n) e =
                EndomorphismObject.power (EndomorphismObject.conj A e) n := by
            exact EndomorphismObject.conj_eq
              ...
              (by
                ...
                exact (EndomorphismObject.conj_pow n A e).symm)
          exact congrArg (fun X => of K X - of K (EndomorphismObject.power A n)) hconj.symm
