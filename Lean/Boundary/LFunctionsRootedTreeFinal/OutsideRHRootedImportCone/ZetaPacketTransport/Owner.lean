...
theorem packetTransportSurface_eq_spectralModel_separatingProbe (f : ZetaAdmissibleFunction) :
    packetTransportSurface f = (spectralModel f, separatingProbe f) :=
  interpolationSurface_eq_spectralModel_separatingProbe f
...
theorem packetTransportSurface_fst_sum {α : Type*} [DecidableEq α] (s : Finset α)
    (f : α → ZetaAdmissibleFunction) :
    (packetTransportSurface (∑ a in s, f a)).1 = ∑ a in s, spectralModel (f a) := by
  exact interpolationSurface_fst_sum s f
...
theorem packetTransportSurface_sum {α : Type*} [DecidableEq α] (s : Finset α)
    (f : α → ZetaAdmissibleFunction) :
    packetTransportSurface (∑ a in s, f a) =
      (∑ a in s, spectralModel (f a), separatingProbe (∑ a in s, f a)) :=
  Prod.ext
    (packetTransportSurface_fst_sum s f)
    (packetTransportSurface_snd (∑ a in s, f a))
...
theorem exists_packetTransportSurface_eval_sample_with_support
    ...
    ∃ f : ZetaAdmissibleFunction,
      ...
      (packetTransportSurface f).2 = separatingProbe f := by
  exact exists_interpolationSurface_eval_sample_with_support S a
...
theorem exists_packetTransportSurface_eval_sample_with_basis_support
    ...
    ∃ F : ∀ _i, ZetaAdmissibleFunction,
      ...
      ∃ f : ZetaAdmissibleFunction,
        ...
        (packetTransportSurface f).1 = spectralModel f ∧
        (packetTransportSurface f).2 = separatingProbe f := by
  exact exists_interpolationSurface_eval_sample_with_basis_support S a
...
theorem exists_packetTransportSurface_eval_sample_with_basis_closedBall_support
    ...
    ∃ F : ∀ _i, ZetaAdmissibleFunction,
      ...
      ∃ f : ZetaAdmissibleFunction,
        ...
        (packetTransportSurface f).1 = spectralModel f ∧
        (packetTransportSurface f).2 = separatingProbe f := by
  exact exists_interpolationSurface_eval_sample_with_basis_closedBall_support S a
