...
abbrev LegacyCompletedRecord := TraceCalc.LayerB.ShadowModel.CompletedRecord

abbrev LegacyCanonicalThresholdCut {n : Nat}
    (r : LegacyCompletedRecord n) (threshold : Nat) :=
  TraceCalc.LayerB.ShadowModel.CanonicalThresholdCut r threshold

def completedRecordThresholdCut
    {reconstructionLength threshold : Nat}
    (packet : LegacyCompletedRecord reconstructionLength)
    (hThreshold : threshold ≤ reconstructionLength) :
...
theorem completedRecordThresholdCut_lower_mem_iff
    {reconstructionLength threshold : Nat}
    (packet : LegacyCompletedRecord reconstructionLength)
    (hThreshold : threshold ≤ reconstructionLength)
    (degree : Fin reconstructionLength) :
    degree ∈ (completedRecordThresholdCut packet hThreshold).lowerSet ↔ degree.val < threshold :=
  (completedRecordThresholdCut packet hThreshold).lower_mem_iff degree

theorem completedRecordThresholdCut_upper_mem_iff
    {reconstructionLength threshold : Nat}
    (packet : LegacyCompletedRecord reconstructionLength)
    (hThreshold : threshold ≤ reconstructionLength)
    (degree : Fin reconstructionLength) :
    degree ∈ (completedRecordThresholdCut packet hThreshold).upperSet ↔ threshold ≤ degree.val :=
  (completedRecordThresholdCut packet hThreshold).upper_mem_iff degree

theorem completedRecordThresholdCut_lower_dependency_closed
    {reconstructionLength threshold : Nat}
    (packet : LegacyCompletedRecord reconstructionLength)
    (hThreshold : threshold ≤ reconstructionLength)
    {j i : Fin reconstructionLength}
    (hj : j ∈ (completedRecordThresholdCut packet hThreshold).lowerSet)
    (hi : i ∈ packet.requires j) :
    i ∈ (completedRecordThresholdCut packet hThreshold).lowerSet :=
  (completedRecordThresholdCut packet hThreshold).lower_dependency_closed hj hi

theorem completedRecordThresholdCut_partition
    {reconstructionLength threshold : Nat}
    (packet : LegacyCompletedRecord reconstructionLength)
    (hThreshold : threshold ≤ reconstructionLength) :
    (completedRecordThresholdCut packet hThreshold).lowerSet ∪
        (completedRecordThresholdCut packet hThreshold).upperSet = Finset.univ :=
  (completedRecordThresholdCut packet hThreshold).partition

theorem completedRecordThresholdCut_disjoint
    {reconstructionLength threshold : Nat}
    (packet : LegacyCompletedRecord reconstructionLength)
    (hThreshold : threshold ≤ reconstructionLength) :
    Disjoint
      (completedRecordThresholdCut packet hThreshold).lowerSet
      (completedRecordThresholdCut packet hThreshold).upperSet :=
  (completedRecordThresholdCut packet hThreshold).disjoint
...
/-- Legacy shadow-model carrier for a morphism between reconstructed completed
records. This remains local to `RecognitionTarget` while real-object transport
replaces the remaining shadow-model cut path. -/
structure LegacyCompletedRecordMorphismCarrier
  {sourceLength targetLength : Nat}
  (source : LegacyCompletedRecord sourceLength)
  (target : LegacyCompletedRecord targetLength) where
  carrier : Type z
  witness : carrier
...
structure FiveFamilyCompactGenerationWitness
  ...
  completedRecordMorphismToFiniteTraceClosureMorphismCarrier :
    {sourceLength targetLength : Nat} →
      {source : LegacyCompletedRecord sourceLength} →
      {target : LegacyCompletedRecord targetLength} →
      LegacyCompletedRecordMorphismCarrier source target →
        FiniteTraceClosureMorphismCarrier
...
def realCompletedRecordToTraceObject
    {trace : TracePresentation.{u, v, w, x, y}}
    {presentation : ClassicalMotivicPresentation trace}
    {ctx : ClassicalBridge.ClassicalComparisonContext.{u, v}}
    (witness : FiveFamilyCompactGenerationWitness trace presentation ctx)
    (record :
      TraceCalc.LayerB.RealObjects.RewriteCalculusSetup.CompletedReconstructionRecord
        witness.canonicalRealReconstructionTransport.setup) :
    trace.TraceObject :=
  witness.canonicalRealReconstructionTransport.toTraceObject record

def realCompletedRecordToClassicalCompactObject
    {trace : TracePresentation.{u, v, w, x, y}}
    {presentation : ClassicalMotivicPresentation trace}
    {ctx : ClassicalBridge.ClassicalComparisonContext.{u, v}}
    (witness : FiveFamilyCompactGenerationWitness trace presentation ctx)
    (record :
      TraceCalc.LayerB.RealObjects.RewriteCalculusSetup.CompletedReconstructionRecord
        witness.canonicalRealReconstructionTransport.setup) :
    presentation.motivicCategory.Object :=
  witness.canonicalRealReconstructionTransport.toClassicalCompactObject record
...
def ofTraceCategoryAndClosure
  ...
  compactGenerationTransport := {
    ...
    finiteTraceClosureToTraceObject := seedTransport.finiteTraceClosureToTraceObject
    ...
    reconstructCompactObject := seedTransport.reconstructCompactObject
    reconstructCompactMorphism := seedTransport.reconstructCompactMorphism
    canonicalRealReconstructionTransport := seedTransport.canonicalRealReconstructionTransport
...
def ofFiveFamilyClosure
  ...
  canonicalRealReconstructionTransport := witness.canonicalRealReconstructionTransport
  reconstructionFromFiniteTraceClosureTarget :=
...
- `compactGeometricGenerationWitness`, `exactTriangulatedWitness`,
  ...
  `CertifiedClassicalDMgmQTarget` contract;
...
structure Campaign11WeightDevissageObligations
  ...
  weightClassNonnegative :
    structuralRecognition.recognition.recognizedCategory.Object → Type z
  canonicalRealReconstructionTransport :
    CanonicalRealReconstructionTransportData
      structuralRecognition.recognition.recognitionInput.tracePresentation
      structuralRecognition.recognition.recognitionInput.classicalPresentation
  ...
  proofRelevantWeightOrthogonalityTarget :
...
structure CertifiedWeightDevissageData
  ...
  proofRelevantWeightOrthogonalityTarget :
    ∀ {sourceObj targetObj : structuralRecognition.recognition.recognizedCategory.Object}
      ...
      (f : structuralRecognition.recognition.recognizedCategory.Hom sourceObj targetObj),
      Prop
  proofRelevantWeightOrthogonality_holds :
    ∀ {sourceObj targetObj : structuralRecognition.recognition.recognizedCategory.Object}
      (separated : separatedWeightRelation sourceObj targetObj)
      (f : structuralRecognition.recognition.recognizedCategory.Hom sourceObj targetObj),
      proofRelevantWeightOrthogonalityTarget separated f
  finiteTraceClosureCompatibilityTarget : Prop
...
/-- A concrete recognized-category morphism together with evidence that it is
the zero morphism. The recognized category layer currently exposes `Hom` but
not a canonical zero morphism operation, so zero detection remains proof
relevant. -/
structure TraceMotivicZeroMorphismData
    (structuralRecognition : DMgmStructuralRecognitionTarget.{u, v, w, x, y, z})
    {X Y : structuralRecognition.recognition.recognizedCategory.Object} where
  morphism : structuralRecognition.recognition.recognizedCategory.Hom X Y
  isZero : Prop

/-- Zero-morphism witness anchored to a specific recognized morphism. -/
structure TraceMotivicZeroMorphismWitness
    (structuralRecognition : DMgmStructuralRecognitionTarget.{u, v, w, x, y, z})
    {X Y : structuralRecognition.recognition.recognizedCategory.Object}
    (f : structuralRecognition.recognition.recognizedCategory.Hom X Y) where
  isZero : Prop

namespace TraceMotivicZeroMorphismWitness

def toData
    {structuralRecognition : DMgmStructuralRecognitionTarget.{u, v, w, x, y, z}}
    {X Y : structuralRecognition.recognition.recognizedCategory.Object}
    {f : structuralRecognition.recognition.recognizedCategory.Hom X Y}
    (witness : TraceMotivicZeroMorphismWitness structuralRecognition f) :
    TraceMotivicZeroMorphismData structuralRecognition (X := X) (Y := Y) where
  morphism := f
  isZero := witness.isZero

@[simp] theorem toData_morphism
    {structuralRecognition : DMgmStructuralRecognitionTarget.{u, v, w, x, y, z}}
    {X Y : structuralRecognition.recognition.recognizedCategory.Object}
    {f : structuralRecognition.recognition.recognizedCategory.Hom X Y}
    (witness : TraceMotivicZeroMorphismWitness structuralRecognition f) :
    witness.toData.morphism = f :=
  rfl

@[simp] theorem toData_isZero
    {structuralRecognition : DMgmStructuralRecognitionTarget.{u, v, w, x, y, z}}
    {X Y : structuralRecognition.recognition.recognizedCategory.Object}
    {f : structuralRecognition.recognition.recognizedCategory.Hom X Y}
    (witness : TraceMotivicZeroMorphismWitness structuralRecognition f) :
    witness.toData.isZero = witness.isZero :=
  rfl

end TraceMotivicZeroMorphismWitness

namespace CanonicalRealReconstructionTransportData

def toRecognizedObject
    (recognition : DMgmRecognitionTarget.{u, v, w, x, y, z})
    (transport : CanonicalRealReconstructionTransportData
      recognition.recognitionInput.tracePresentation
      recognition.recognitionInput.classicalPresentation)
    (record :
      TraceCalc.LayerB.RealObjects.RewriteCalculusSetup.CompletedReconstructionRecord
        transport.setup) :
    recognition.recognizedCategory.Object :=
  classicalObjectToRecognized recognition (transport.toClassicalCompactObject record)

end CanonicalRealReconstructionTransportData

namespace LegacyShadowTransport

def completedRecordToFiniteTraceClosureCarrier
    {structuralRecognition : DMgmStructuralRecognitionTarget.{u, v, w, x, y, z}}
    {traceCategory :
    ...
    {compactGenerationTransport :
      FiveFamilyCompactGenerationWitness
        ...
        structuralRecognition.recognition.recognitionInput.classicalPresentation.classicalContext}
    {reconstructionLength : Nat}
    (completedRecord : LegacyCompletedRecord reconstructionLength) :
...
def completedRecordToRecognizedObject
    {structuralRecognition : DMgmStructuralRecognitionTarget.{u, v, w, x, y, z}}
    {traceCategory :
      ClassicalPeriods.TraceCategoryStructure
        structuralRecognition.recognition.recognitionInput.classicalPresentation.classicalContext}
...
def recognizedMorphismFromCompletedRecordMorphism
    ...
    {source : LegacyCompletedRecord sourceLength}
    {target : LegacyCompletedRecord targetLength}
    (morphismCarrier : LegacyCompletedRecordMorphismCarrier source target) :
    structuralRecognition.recognition.recognizedCategory.Hom
      (completedRecordToRecognizedObject
...
def canonicalThresholdCutTotalCarrier
    {structuralRecognition : DMgmStructuralRecognitionTarget.{u, v, w, x, y, z}}
    {traceCategory :
      ClassicalPeriods.TraceCategoryStructure
        structuralRecognition.recognition.recognitionInput.classicalPresentation.classicalContext}
    ...
    {reconstructionLength threshold : Nat}
    {completedRecord : LegacyCompletedRecord reconstructionLength}
    (_cut : LegacyCanonicalThresholdCut completedRecord threshold) :
    compactGenerationTransport.FiniteTraceClosureCarrier :=
  completedRecordToFiniteTraceClosureCarrier
    (structuralRecognition := structuralRecognition)
    (traceCategory := traceCategory)
...
def canonicalThresholdCutTotalRecognizedObject
    {structuralRecognition : DMgmStructuralRecognitionTarget.{u, v, w, x, y, z}}
    {traceCategory :
      ClassicalPeriods.TraceCategoryStructure
        structuralRecognition.recognition.recognitionInput.classicalPresentation.classicalContext}
...
def canonicalThresholdCutLowerCarrier
    {structuralRecognition : DMgmStructuralRecognitionTarget.{u, v, w, x, y, z}}
    {traceCategory :
      ClassicalPeriods.TraceCategoryStructure
        structuralRecognition.recognition.recognitionInput.classicalPresentation.classicalContext}
    ...
    {reconstructionLength threshold : Nat}
    {completedRecord : LegacyCompletedRecord reconstructionLength}
    (cut : LegacyCanonicalThresholdCut completedRecord threshold) :
    compactGenerationTransport.FiniteTraceClosureCarrier :=
  completedRecordToFiniteTraceClosureCarrier
    (structuralRecognition := structuralRecognition)
    (traceCategory := traceCategory)
...
def canonicalThresholdCutUpperCarrier
    {structuralRecognition : DMgmStructuralRecognitionTarget.{u, v, w, x, y, z}}
    {traceCategory :
      ClassicalPeriods.TraceCategoryStructure
        structuralRecognition.recognition.recognitionInput.classicalPresentation.classicalContext}
...
def canonicalThresholdCutLowerRecognizedObject
    {structuralRecognition : DMgmStructuralRecognitionTarget.{u, v, w, x, y, z}}
    {traceCategory :
      ClassicalPeriods.TraceCategoryStructure
        structuralRecognition.recognition.recognitionInput.classicalPresentation.classicalContext}
...
def canonicalThresholdCutUpperRecognizedObject
    {structuralRecognition : DMgmStructuralRecognitionTarget.{u, v, w, x, y, z}}
    {traceCategory :
      ClassicalPeriods.TraceCategoryStructure
        structuralRecognition.recognition.recognitionInput.classicalPresentation.classicalContext}
...
def canonicalThresholdCutLowerInclusionCarrier
    {reconstructionLength threshold : Nat}
    {completedRecord : LegacyCompletedRecord reconstructionLength}
    (cut : LegacyCanonicalThresholdCut completedRecord threshold) :
  LegacyCompletedRecordMorphismCarrier
...
def canonicalThresholdCutUpperProjectionCarrier
    {reconstructionLength threshold : Nat}
    {completedRecord : LegacyCompletedRecord reconstructionLength}
    (cut : LegacyCanonicalThresholdCut completedRecord threshold) :
  LegacyCompletedRecordMorphismCarrier
...
def canonicalThresholdCutLowerInclusionRecognized
    {structuralRecognition : DMgmStructuralRecognitionTarget.{u, v, w, x, y, z}}
    {traceCategory :
      ClassicalPeriods.TraceCategoryStructure
        structuralRecognition.recognition.recognitionInput.classicalPresentation.classicalContext}
    ...
    (cut : LegacyCanonicalThresholdCut completedRecord threshold) :
    structuralRecognition.recognition.recognizedCategory.Hom
      (canonicalThresholdCutLowerRecognizedObject
        (structuralRecognition := structuralRecognition)
        (traceCategory := traceCategory)
        (assignmentTable := assignmentTable)
        (closure := closure)
        (compactGenerationTransport := compactGenerationTransport)
        cut)
      (canonicalThresholdCutTotalRecognizedObject
        (structuralRecognition := structuralRecognition)
        (traceCategory := traceCategory)
        (assignmentTable := assignmentTable)
        (closure := closure)
        (compactGenerationTransport := compactGenerationTransport)
        cut) :=
  recognizedMorphismFromCompletedRecordMorphism
    (structuralRecognition := structuralRecognition)
    (traceCategory := traceCategory)
    (assignmentTable := assignmentTable)
    (closure := closure)
    (compactGenerationTransport := compactGenerationTransport)
    (canonicalThresholdCutLowerInclusionCarrier cut)

def canonicalThresholdCutUpperProjectionRecognized
    {structuralRecognition : DMgmStructuralRecognitionTarget.{u, v, w, x, y, z}}
    {traceCategory :
      ClassicalPeriods.TraceCategoryStructure
        structuralRecognition.recognition.recognitionInput.classicalPresentation.classicalContext}
    ...
    structuralRecognition.recognition.recognizedCategory.Hom
      (canonicalThresholdCutTotalRecognizedObject
        ...
        (compactGenerationTransport := compactGenerationTransport)
        cut)
      (canonicalThresholdCutUpperRecognizedObject
        (structuralRecognition := structuralRecognition)
        (traceCategory := traceCategory)
        (assignmentTable := assignmentTable)
        (closure := closure)
  ...
  recognizedMorphismFromCompletedRecordMorphism
    ...
    (closure := closure)
    (compactGenerationTransport := compactGenerationTransport)
    (canonicalThresholdCutUpperProjectionCarrier cut)

end LegacyShadowTransport
...
def centeredPacketDegree (degree threshold : Nat) : Int :=
  Int.ofNat degree - Int.ofNat threshold

end TraceNativeWeightDevissageData

namespace TraceComplexityWeightClassData

def FiniteTraceClosureFromPacketsBelowThreshold
    {structuralRecognition : DMgmStructuralRecognitionTarget.{u, v, w, x, y, z}}
    {traceCategory :
      ClassicalPeriods.TraceCategoryStructure
        structuralRecognition.recognition.recognitionInput.classicalPresentation.classicalContext}
  ...
  Σ threshold : Nat,
    Σ cut : LegacyCanonicalThresholdCut completedRecord threshold,
      ULift.{z, 0}
        (PLift
          (∀ i : Fin reconstructionLength,
            i ∈ cut.lowerSet →
              TraceNativeWeightDevissageData.centeredPacketDegree i.val threshold ≤ 0)) ×
        ULift.{z, 0}
          (PLift
            (LegacyShadowTransport.canonicalThresholdCutLowerRecognizedObject
                (structuralRecognition := structuralRecognition)
                (traceCategory := traceCategory)
                (assignmentTable := assignmentTable)
                (closure := closure)
...
def FiniteTraceClosureFromPacketsAboveThreshold
    {structuralRecognition : DMgmStructuralRecognitionTarget.{u, v, w, x, y, z}}
    {traceCategory :
      ClassicalPeriods.TraceCategoryStructure
        structuralRecognition.recognition.recognitionInput.classicalPresentation.classicalContext}
    ...
    Σ cut : LegacyCanonicalThresholdCut completedRecord threshold,
      ULift.{z, 0}
        (PLift
          (∀ i : Fin reconstructionLength,
            i ∈ cut.upperSet →
              0 ≤ TraceNativeWeightDevissageData.centeredPacketDegree i.val threshold)) ×
        ULift.{z, 0}
          (PLift
            (LegacyShadowTransport.canonicalThresholdCutUpperRecognizedObject
                (structuralRecognition := structuralRecognition)
                (traceCategory := traceCategory)
                (assignmentTable := assignmentTable)
                (closure := closure)
...
def weightClassNonpositive
  ...
  (Σ family : FiveFamilyGeneratorIndex,
    ...
    data.FiniteTraceClosureFromPacketsBelowThreshold obj
...
def weightClassNonnegative
  ...
  (Σ family : FiveFamilyGeneratorIndex,
      PLift (pureGenerators.recognizedGeneratorObject family = obj) ×
        data.pureGeneratorBaseClassWitness family) ⊕
    data.FiniteTraceClosureFromPacketsAboveThreshold obj

def finiteTraceClosure_preserves_weightClassNonpositive
    {structuralRecognition : DMgmStructuralRecognitionTarget.{u, v, w, x, y, z}}
    {traceCategory :
      ClassicalPeriods.TraceCategoryStructure
        structuralRecognition.recognition.recognitionInput.classicalPresentation.classicalContext}
    ...
    (data :
      TraceComplexityWeightClassData structuralRecognition traceCategory assignmentTable closure
        compactGenerationTransport pureGenerators reconstructionLength completedRecord)
    {obj : structuralRecognition.recognition.recognizedCategory.Object} :
    data.FiniteTraceClosureFromPacketsBelowThreshold obj →
      data.weightClassNonpositive obj :=
  Sum.inr

def finiteTraceClosure_preserves_weightClassNonnegative
    {structuralRecognition : DMgmStructuralRecognitionTarget.{u, v, w, x, y, z}}
    {traceCategory :
      ClassicalPeriods.TraceCategoryStructure
        structuralRecognition.recognition.recognitionInput.classicalPresentation.classicalContext}
    ...
    {obj : structuralRecognition.recognition.recognizedCategory.Object} :
    data.FiniteTraceClosureFromPacketsAboveThreshold obj →
      data.weightClassNonnegative obj :=
  Sum.inr
...
def ofReconstruction
  ...
  normalizationInvariantTarget :=
    CanonicalRealReconstructionTransportData.reconstructionCompatibilityTarget
      compactGenerationTransport.canonicalRealReconstructionTransport
  normalizationInvariant_holds :=
    CanonicalRealReconstructionTransportData.reconstructionCompatibility_holds
      compactGenerationTransport.canonicalRealReconstructionTransport
...
structure TraceWeightOrthogonalityData
  ...
  separatedWeightWitness :
    {sourceObj targetObj : structuralRecognition.recognition.recognizedCategory.Object} →
      weightClasses.weightClassNonpositive sourceObj →
      weightClasses.weightClassNonnegative targetObj →
      SeparatedWeightRelation sourceObj targetObj
  ...
  traceDevissageZeroWitnessTarget :
    ∀ {sourceObj targetObj : structuralRecognition.recognition.recognizedCategory.Object}
      (separated : SeparatedWeightRelation sourceObj targetObj)
      (f : structuralRecognition.recognition.recognizedCategory.Hom sourceObj targetObj),
      Prop
  traceDevissageZeroWitness_holds :
    ∀ {sourceObj targetObj : structuralRecognition.recognition.recognizedCategory.Object}
      (separated : SeparatedWeightRelation sourceObj targetObj)
      (f : structuralRecognition.recognition.recognizedCategory.Hom sourceObj targetObj),
      traceDevissageZeroWitnessTarget separated f
  finiteTraceClosureCompatibilityTarget : Prop
  finiteTraceClosureCompatibility_holds : finiteTraceClosureCompatibilityTarget
  compatibilityWithPureGeneratorsTarget : Prop
  compatibilityWithPureGenerators_holds : compatibilityWithPureGeneratorsTarget
...
def ofReconstruction
  ...
  boundarySupportObstructionWitness := fun {_ _} _ =>
    PUnit
  separatedWeightWitness := fun {_ _} sourceWitness targetWitness =>
  ...
  traceDevissageOrthogonality_holds := by
    ...
    exact pureGenerators.boundarySupportGluingCompatibility_holds
  traceDevissageZeroWitnessTarget := fun _ f =>
    Nonempty (TraceMotivicZeroMorphismWitness structuralRecognition f)
  traceDevissageZeroWitness_holds := by
    intro sourceObj targetObj separated f
    exact ⟨{ isZero :=
      assignmentTable.assignmentCompatibilityTarget ∧
        closure.closureComparisonTarget ∧
        traceCategory.categoricalShadowTarget }⟩
...
def canonicalRealCompletedRecordToRecognizedObject
    {structuralRecognition : DMgmStructuralRecognitionTarget.{u, v, w, x, y, z}}
    {traceCategory :
      ClassicalPeriods.TraceCategoryStructure
        structuralRecognition.recognition.recognitionInput.classicalPresentation.classicalContext}
    ...
    (record :
      TraceCalc.LayerB.RealObjects.RewriteCalculusSetup.CompletedReconstructionRecord
        traceNative.canonicalRealReconstructionTransport.setup) :
    structuralRecognition.recognition.recognizedCategory.Object :=
  CanonicalRealReconstructionTransportData.toRecognizedObject
    structuralRecognition.recognition traceNative.canonicalRealReconstructionTransport record

def canonicalRealCompletedRecordToTraceObject
    {structuralRecognition : DMgmStructuralRecognitionTarget.{u, v, w, x, y, z}}
    {traceCategory :
      ClassicalPeriods.TraceCategoryStructure
        structuralRecognition.recognition.recognitionInput.classicalPresentation.classicalContext}
    ...
    (record :
      TraceCalc.LayerB.RealObjects.RewriteCalculusSetup.CompletedReconstructionRecord
        traceNative.canonicalRealReconstructionTransport.setup) :
    structuralRecognition.recognition.recognitionInput.tracePresentation.TraceObject :=
  traceNative.canonicalRealReconstructionTransport.toTraceObject record
...
def canonicalPacketCut
    ...
    TraceCalc.LayerB.ShadowModel.CanonicalThresholdCut traceNative.completedRecord threshold :=
  TraceCalc.LayerB.ShadowModel.CanonicalThresholdCut.ofLe
    traceNative.completedRecord threshold hThreshold

def canonicalPacketCutTotalRecognizedObject
    {structuralRecognition : DMgmStructuralRecognitionTarget.{u, v, w, x, y, z}}
    {traceCategory :
      ClassicalPeriods.TraceCategoryStructure
        structuralRecognition.recognition.recognitionInput.classicalPresentation.classicalContext}
    ...
    (traceNative :
      TraceNativeWeightDevissageData structuralRecognition traceCategory assignmentTable closure
        compactGenerationTransport)
    {threshold : Nat}
    (cut : LegacyCanonicalThresholdCut traceNative.completedRecord threshold) :
    structuralRecognition.recognition.recognizedCategory.Object :=
  LegacyShadowTransport.canonicalThresholdCutTotalRecognizedObject
    (structuralRecognition := structuralRecognition)
    (traceCategory := traceCategory)
    (assignmentTable := assignmentTable)
    (closure := closure)
...
def canonicalPacketCutLowerRecognizedObject
    {structuralRecognition : DMgmStructuralRecognitionTarget.{u, v, w, x, y, z}}
    {traceCategory :
      ClassicalPeriods.TraceCategoryStructure
        structuralRecognition.recognition.recognitionInput.classicalPresentation.classicalContext}
...
def canonicalPacketCutUpperRecognizedObject
    {structuralRecognition : DMgmStructuralRecognitionTarget.{u, v, w, x, y, z}}
    {traceCategory :
      ClassicalPeriods.TraceCategoryStructure
        structuralRecognition.recognition.recognitionInput.classicalPresentation.classicalContext}
...
def canonicalPacketCutLowerInclusionRecognized
    {structuralRecognition : DMgmStructuralRecognitionTarget.{u, v, w, x, y, z}}
    {traceCategory :
      ClassicalPeriods.TraceCategoryStructure
        structuralRecognition.recognition.recognitionInput.classicalPresentation.classicalContext}
    ...
    (cut : LegacyCanonicalThresholdCut traceNative.completedRecord threshold) :
    structuralRecognition.recognition.recognizedCategory.Hom
      (canonicalPacketCutLowerRecognizedObject traceNative cut)
      (canonicalPacketCutTotalRecognizedObject traceNative cut) :=
  LegacyShadowTransport.canonicalThresholdCutLowerInclusionRecognized
    (structuralRecognition := structuralRecognition)
    (traceCategory := traceCategory)
    (assignmentTable := assignmentTable)
    (closure := closure)
...
def canonicalPacketCutUpperProjectionRecognized
    {structuralRecognition : DMgmStructuralRecognitionTarget.{u, v, w, x, y, z}}
    {traceCategory :
      ClassicalPeriods.TraceCategoryStructure
        structuralRecognition.recognition.recognitionInput.classicalPresentation.classicalContext}
    ...
    structuralRecognition.recognition.recognizedCategory.Hom
      (canonicalPacketCutTotalRecognizedObject traceNative cut)
      (canonicalPacketCutUpperRecognizedObject traceNative cut) :=
  LegacyShadowTransport.canonicalThresholdCutUpperProjectionRecognized
    (structuralRecognition := structuralRecognition)
    (traceCategory := traceCategory)
    (assignmentTable := assignmentTable)
    (closure := closure)
...
def canonicalPacketCutCofiberSequenceData
    {structuralRecognition : DMgmStructuralRecognitionTarget.{u, v, w, x, y, z}}
    {traceCategory :
      ClassicalPeriods.TraceCategoryStructure
        structuralRecognition.recognition.recognitionInput.classicalPresentation.classicalContext}
    ...
    (traceNative :
      TraceNativeWeightDevissageData structuralRecognition traceCategory assignmentTable closure
        compactGenerationTransport)
    {threshold : Nat}
    (cut : TraceCalc.LayerB.ShadowModel.CanonicalThresholdCut traceNative.completedRecord threshold) :
    Type :=
  TraceCalc.LayerB.ShadowModel.CanonicalThresholdCut.CofiberSequenceData cut

theorem canonicalLowerCut_supportBelow
    {structuralRecognition : DMgmStructuralRecognitionTarget.{u, v, w, x, y, z}}
    {traceCategory :
      ClassicalPeriods.TraceCategoryStructure
        structuralRecognition.recognition.recognitionInput.classicalPresentation.classicalContext}
    ...
    (threshold : Nat)
    (hThreshold : threshold ≤ traceNative.reconstructionLength)
    (i : Fin traceNative.reconstructionLength)
    (hi : i ∈ (TraceCalc.LayerB.ShadowModel.CanonicalThresholdCut.ofLe
      traceNative.completedRecord threshold hThreshold).lowerSet) :
    centeredPacketDegree i.val threshold ≤ 0 := by
  exact sub_nonpos.mpr <| Int.ofNat_le.mpr <|
    Nat.le_of_lt <|
      ((TraceCalc.LayerB.ShadowModel.CanonicalThresholdCut.ofLe
        traceNative.completedRecord threshold hThreshold).lower_mem_iff i).mp hi

theorem canonicalUpperCut_supportAbove
    {structuralRecognition : DMgmStructuralRecognitionTarget.{u, v, w, x, y, z}}
    {traceCategory :
      ClassicalPeriods.TraceCategoryStructure
        structuralRecognition.recognition.recognitionInput.classicalPresentation.classicalContext}
    ...
    (i : Fin traceNative.reconstructionLength)
    (hi : i ∈ (TraceCalc.LayerB.ShadowModel.CanonicalThresholdCut.ofLe
      traceNative.completedRecord threshold hThreshold).upperSet) :
    0 ≤ centeredPacketDegree i.val threshold := by
  exact sub_nonneg.mpr <| Int.ofNat_le.mpr <|
    ((TraceCalc.LayerB.ShadowModel.CanonicalThresholdCut.ofLe
      traceNative.completedRecord threshold hThreshold).upper_mem_iff i).mp hi

def canonicalLowerCut_weightClassNonpositive
    {structuralRecognition : DMgmStructuralRecognitionTarget.{u, v, w, x, y, z}}
    {traceCategory :
      ClassicalPeriods.TraceCategoryStructure
        structuralRecognition.recognition.recognitionInput.classicalPresentation.classicalContext}
    ...
    (traceNative :
      TraceNativeWeightDevissageData structuralRecognition traceCategory assignmentTable closure
        compactGenerationTransport)
    {threshold : Nat}
    (cut : LegacyCanonicalThresholdCut
      traceNative.completedRecord threshold) :
    traceNative.weightClasses.weightClassNonpositive
      (canonicalPacketCutLowerRecognizedObject traceNative cut) :=
  traceNative.weightClasses.finiteTraceClosure_preserves_weightClassNonpositive
    ⟨threshold,
      cut,
        ULift.up <| PLift.up <| show ∀ i : Fin traceNative.reconstructionLength,
            i ∈ cut.lowerSet →
              centeredPacketDegree i.val threshold ≤ 0 from by
          intro i hi
          exact sub_nonpos.mpr <| Int.ofNat_le.mpr <|
            Nat.le_of_lt <| (cut.lower_mem_iff i).mp hi,
        ULift.up <| PLift.up rfl⟩

def canonicalUpperCut_weightClassNonnegative
    {structuralRecognition : DMgmStructuralRecognitionTarget.{u, v, w, x, y, z}}
    {traceCategory :
      ClassicalPeriods.TraceCategoryStructure
        structuralRecognition.recognition.recognitionInput.classicalPresentation.classicalContext}
    ...
    (cut : LegacyCanonicalThresholdCut
      traceNative.completedRecord threshold) :
    traceNative.weightClasses.weightClassNonnegative
      (canonicalPacketCutUpperRecognizedObject traceNative cut) :=
  traceNative.weightClasses.finiteTraceClosure_preserves_weightClassNonnegative
    ⟨threshold,
      cut,
        ULift.up <| PLift.up <| show ∀ i : Fin traceNative.reconstructionLength,
            i ∈ cut.upperSet →
              0 ≤ centeredPacketDegree i.val threshold from by
          intro i hi
          exact sub_nonneg.mpr <| Int.ofNat_le.mpr <|
            (cut.upper_mem_iff i).mp hi,
        ULift.up <| PLift.up rfl⟩

def canonicalPacketCutSeparatedWeightData
    {structuralRecognition : DMgmStructuralRecognitionTarget.{u, v, w, x, y, z}}
    {traceCategory :
      ClassicalPeriods.TraceCategoryStructure
        structuralRecognition.recognition.recognitionInput.classicalPresentation.classicalContext}
    ...
    (cut : LegacyCanonicalThresholdCut
      traceNative.completedRecord threshold) :
    traceNative.weightOrthogonality.SeparatedWeightRelation
      (LegacyShadowTransport.canonicalThresholdCutLowerRecognizedObject
        (structuralRecognition := structuralRecognition)
        (traceCategory := traceCategory)
        (assignmentTable := assignmentTable)
  ...
  traceNative.weightOrthogonality.separatedWeightWitness
    (canonicalLowerCut_weightClassNonpositive traceNative cut)
    (canonicalUpperCut_weightClassNonnegative traceNative cut)
...
def ofTraceNativeData
  ...
  canonicalRealReconstructionTransport := traceNative.canonicalRealReconstructionTransport
  ...
  separatedWeightRelation := traceNative.weightOrthogonality.SeparatedWeightRelation
  boundedWeightDecompositionTarget :=
    traceNative.canonicalRealReconstructionTransport.reconstructionCompatibilityTarget
  boundedWeightDecomposition_holds :=
    traceNative.canonicalRealReconstructionTransport.reconstructionCompatibility_holds
  ...
  proofRelevantWeightOrthogonalityTarget := fun {sourceObj targetObj} separated f =>
    traceNative.weightOrthogonality.traceDevissageZeroWitnessTarget separated f
  proofRelevantWeightOrthogonality_holds := by
    intro sourceObj targetObj separated f
    exact traceNative.weightOrthogonality.traceDevissageZeroWitness_holds separated f
  finiteTraceClosureCompatibilityTarget :=
...
def ofCertifiedWeightDevissageData
  ...
  canonicalRealReconstructionTransport := certified.canonicalRealReconstructionTransport
  ...
  separatedWeightRelation := certified.separatedWeightRelation
  ...
  proofRelevantWeightOrthogonalityTarget := certified.proofRelevantWeightOrthogonalityTarget
