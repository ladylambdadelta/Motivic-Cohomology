...
/-- A half-open logarithmic real-phase interval estimate at the endpoint
constant follows from corresponding closed-interval estimates, with the empty
interval handled constructively. -/
theorem Complex.logarithmicPhaseRealPhase_Ico_twenty_bound_of_Icc_bounds
    (t : ℝ)
    (ht : 1 ≤ ‖t‖)
    {b c d : ℕ}
    (hd_right : d ≤ b + 1)
...
/-- Half-open logarithmic B-process interval reduction at the endpoint
constant from closed subinterval estimates. -/
theorem Complex.logarithmicPhaseRealPhase_Ico_twenty_bProcess_of_Icc_bounds
    (t : ℝ)
    (ht : 1 ≤ ‖t‖)
    {b c d : ℕ}
    (hd_right : d ≤ b + 1)
