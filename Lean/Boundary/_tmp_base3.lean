...
example (A : Type u) [CommRing A] :
  ...
  have hdet0 : LinearMap.det hQ.differential = (1 : A) := by
    exact (LinearMap.det_eq_one_of_subsingleton (R := A) (M := PEmpty → A) hQ.differential)
  have hdet : (MvPolynomial.map (RingHom.id A)) (LinearMap.det hQ.differential) = (1 : MvPolynomial (Fin 1) A) := by
    rw [hdet0]
