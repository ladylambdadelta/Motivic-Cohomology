...
theorem poleClearedRiemannZeta_centralStrip_verticalTail_finiteOrder_growth_from_boundary_inputs :
    ∃ A : ℝ, ∃ B : ℝ, ∃ m : ℕ,
      ...
      ∀ z : ℂ,
        ...
        ‖poleClearedRiemannZeta z‖ ≤
          A * Real.exp (B * (1 + ‖z‖) ^ m) :=
  match poleClearedRiemannZeta_centralStrip_verticalTail_PL_input_package with
...
theorem poleClearedRiemannZeta_rightCriticalStrip_ordinaryFiniteOrder_growth_from_functionalEquation_and_EulerMaclaurin :
    ∃ A : ℝ, ∃ B : ℝ, ∃ m : ℕ,
      ...
      ∀ z : ℂ,
        ...
        ‖poleClearedRiemannZeta z‖ ≤
          A * Real.exp (B * (1 + ‖z‖) ^ m) :=
  match poleClearedRiemannZeta_globalFiniteOrder_growth_from_functionalEquation_and_EulerMaclaurin with
  | ⟨A, B, m, hA, hB, hbound⟩ =>
...
theorem riemannZeta_rightCriticalStrip_poleCleared_verticalTail_growth_bound :
    ∃ A : ℝ, ∃ B : ℝ, ∃ m : ℕ,
      ...
      ∀ z : ℂ,
        ...
        ‖(z - 1) * riemannZeta z‖ ≤
          A * Real.exp (B * (1 + ‖z‖) ^ m) := by
  match poleClearedRiemannZeta_rightCriticalStrip_verticalTail_growth_bound with
  | ⟨A, B, m, hA, hB, hbound⟩ =>
      exact
        ⟨A, B, m, hA, hB,
          fun z hz0 hz2 hzim =>
