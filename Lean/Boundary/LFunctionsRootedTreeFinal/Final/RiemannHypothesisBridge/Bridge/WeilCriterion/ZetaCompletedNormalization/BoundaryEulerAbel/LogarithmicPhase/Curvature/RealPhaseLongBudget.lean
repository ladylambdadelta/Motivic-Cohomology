...
/-- Twentieth-budget stationary sample-union control plus explicit
first-derivative data on endpoint subintervals assemble into the long
real-phase block estimate. -/
theorem Complex.logarithmicPhaseRealPhase_long_packet_budget_of_stationaryFamilyTwenty_endpoint_firstDerivative_data
    (t : ℝ)
    (ht : 1 ≤ ‖t‖)
    (ht_nonneg : 0 ≤ t)
    {a b : ℕ}
  ...
  have hstationary :
      ‖∑ m ∈ Complex.logarithmicPhaseRealPhase_stationaryActiveDerivPackets t a b,
        Complex.realPhase_secondDerivative_vdc_packetSum
          ...
          a b m‖ ≤
        20 * ((((b + 1 : ℕ) : ℝ) / ‖t‖ + Real.sqrt (1 + ‖t‖))) :=
    Complex.logarithmicPhaseRealPhase_stationaryPacketContribution_le_twentyTarget_of_familyUnion_twenty
      t hstationary_family
  exact
    Complex.logarithmicPhaseRealPhase_long_packet_budget_of_endpoint_firstDerivative_data
      t ht ht_nonneg ha hab hstationary
      hleft_reduced hleft_sep hfar_reduced hfar_sep
