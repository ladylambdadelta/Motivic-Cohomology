...
/-- The complex numeral `2` is nonzero. -/
theorem complex_two_ne_zero : (2 : ℂ) ≠ 0 :=
  two_ne_zero

/-- The complex numeral `2` has positive real part. -/
theorem complex_two_re_pos : 0 < (2 : ℂ).re :=
  zero_lt_two

/-- The complex numeral `2` is not `1`. -/
theorem complex_two_ne_one : (2 : ℂ) ≠ 1 :=
...
theorem completedRiemannZeta_eventuallyEq_zero_on_puncturedPlane
  ...
  have hU : IsPreconnected U := by
    have hcount : ({0, 1} : Set ℂ).Countable := by
      exact ((Set.countable_singleton (1 : ℂ)).insert (0 : ℂ))
...
theorem completedRiemannZeta_nonzero_two : completedRiemannZeta (2 : ℂ) ≠ 0 := by
  have hcomp :
    ...
    completedRiemannZeta_eq_riemannZeta_mul_gamma
      complex_two_ne_zero
      (Gammaℝ_ne_zero_of_re_pos (2 : ℂ) complex_two_re_pos)
  have hzeta2 : riemannZeta (2 : ℂ) ≠ 0 := by
    intro hzeta
    have hnorm := riemannZeta_two
    have hnorm_zero :
        (0 : ℂ) = ((Real.pi : ℂ) ^ 2 / 6) :=
      Eq.trans hzeta.symm hnorm
    have hpi : ((Real.pi : ℂ) ^ 2 / 6) ≠ 0 := by
      have hpi_complex : (Real.pi : ℂ) ≠ 0 :=
        Complex.ofReal_ne_zero.mpr Real.pi_ne_zero
      have hpi_sq : (Real.pi : ℂ) ^ 2 ≠ 0 :=
        pow_ne_zero 2 hpi_complex
      have hsix : (6 : ℂ) ≠ 0 :=
        OfNat.ofNat_ne_zero 6
      exact div_ne_zero hpi_sq hsix
    exact hpi hnorm_zero.symm
  intro h
  have hprod : riemannZeta (2 : ℂ) * Gammaℝ (2 : ℂ) = 0 := by
    exact hcomp.symm.trans h
  match mul_eq_zero.mp hprod with
  | Or.inl hzeta => exact hzeta2 hzeta
  | Or.inr hGamma =>
      exact (Gammaℝ_ne_zero_of_re_pos (2 : ℂ) complex_two_re_pos) hGamma
...
theorem completedRiemannZeta_not_eventually_zero
  ...
  have h2 : (2 : ℂ) ≠ 0 ∧ (2 : ℂ) ≠ 1 := by
    constructor
    · exact complex_two_ne_zero
    · exact complex_two_ne_one
...
theorem completedRiemannZeta_eventually_ne_zero_of_zero
  ...
  have hA : AnalyticAt ℂ completedRiemannZeta z := by
    exact Complex.analyticAt_iff_eventually_differentiableAt.2 <| by
      ...
      exact differentiableAt_completedZeta hy0 hy1
  exact
    match hA.eventually_eq_zero_or_eventually_ne_zero with
    | Or.inl hzero =>
        False.elim (completedRiemannZeta_not_eventually_zero z hz0 hz1 hzero)
    | Or.inr hne => hne
