...
private theorem frontierWordEquiv_preserves_correctedResidueCanonicalFrontierWord
  ...
  let BC := concreteBoundaryAdminCanonicalizeCongr_closed RI
  obtain ⟨⟨n₁, X₁, Y₁, ports₁, pkts₁, dep₁, att₁, tensor₁, key₁⟩⟩ := w₁
  obtain ⟨⟨n₂, X₂, Y₂, ports₂, pkts₂, dep₂, att₂, tensor₂, key₂⟩⟩ := w₂
  obtain rfl := h.n_eq
  have hX : X₁ = X₂ := h.X_eq
  have hY : Y₁ = Y₂ := by
    simpa [concreteClosedBoundaryData] using BC.canonicalizeY_congr Y₁ Y₂ h.Y_rel
...
consumer-facing `FrontierWord.Equiv`, even though completeness for that broader
...
reconstruction engine can already use the corrected-residue CanNF on canonical
frontier observations in the sound direction. This does not claim the complete
record-equivalence iff yet. -/
theorem correctedResidueEngineCanNF_sound_under_admin_equiv
    ...
    correctedResidueCanNF RI
      ...
      correctedResidueCanNF RI
        (canonicalFrontierWord
          (FrontierObservation.ofChain c₂ d)) :=
  let N : FrontierWordSoundNormalizer (concreteBoundaryMinimalSetup RI) :=
    correctedResidueFrontierWordSoundNormalizer RI
  N.sound (eng.canonical_word_admin_stable h)
...
/-- Honest Campaign-3 packaging theorem: since no broader explicit full
trace/frontier equality relation on `TraceFrontierWord` exists yet,
`GeneratedTraceFrontierEquiv` is the current full generated equality candidate,
and it factors through corrected-residue CanNF equality. -/
...
theorem holographicReconstruction_preservesCorrectedResidueCanNF_of_zeroBoundaries
    ...
    correctedResidueCanNF RI (Drec.toFrontierWord R₁) =
      correctedResidueCanNF RI (Drec.toFrontierWord R₂) := by
  let hGenerated :=
    holographicReconstruction_generatedTraceFrontierEquiv_of_zeroBoundaries
      RI D Drec visibleBoundary hVisibleToFrontier hSourceZero hTargetZero hVisible
  simpa using
