...
theorem zetaCompletedExplicitFormulaCorrectionRightZeroPoleAffineKernelMajorant_two_integrable
  ...
  have hfun :
    ...
    funext t
    exact zetaCompletedExplicitFormulaCorrectionRightZeroPoleAffineKernelMajorant_two_eq_rpow
      f F h t
  exact Eq.subst
...
theorem zetaCompletedExplicitFormulaCorrectionLeftZeroPoleAffineKernelMajorant_two_integrable
  ...
  have hfun :
      ...
      (fun t : ℝ => A * C * (1 + ‖t‖) ^ (-(2 : ℝ))) := by
    funext t
    exact zetaCompletedExplicitFormulaCorrectionLeftZeroPoleAffineKernelMajorant_two_eq_rpow
      f F h t
  exact Eq.subst
    (motive := fun φ : ℝ → ℝ => Integrable φ (volume : Measure ℝ))
    hfun.symm
