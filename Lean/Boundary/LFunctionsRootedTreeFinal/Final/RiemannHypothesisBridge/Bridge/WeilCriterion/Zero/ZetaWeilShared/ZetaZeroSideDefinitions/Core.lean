...
theorem centeredCompletedRiemannZeta_not_eventually_zero_at_zero_of_ne_shiftedPoles
  ...
  have hs0 : (1 / 2 : ℂ) + z ≠ 0 := by
    ...
    have hz_eq : z = -(1 / 2 : ℂ) := by
      calc
        z = ((1 / 2 : ℂ) + z) - (1 / 2 : ℂ) :=
          (complex_add_sub_half z).symm
        _ = 0 - (1 / 2 : ℂ) := by
          exact congrArg (fun w : ℂ => w - (1 / 2 : ℂ)) h
        _ = -(1 / 2 : ℂ) := complex_zero_sub_half
    exact hzneg hz_eq
  have hs1 : (1 / 2 : ℂ) + z ≠ 1 := by
    intro h
    have hz_eq : z = (1 / 2 : ℂ) := by
      have hz_sub : z - (1 / 2 : ℂ) = 0 := by
        exact complex_sub_half_eq_zero_of_half_add_eq_one h
      calc
        z = (z - (1 / 2 : ℂ)) + (1 / 2 : ℂ) :=
          (complex_sub_add_half z).symm
        _ = 0 + (1 / 2 : ℂ) := by
          exact congrArg (fun w : ℂ => w + (1 / 2 : ℂ)) hz_sub
        _ = (1 / 2 : ℂ) := zero_add (1 / 2 : ℂ)
    exact hzpos hz_eq
  have hshift_zero :
    ...
    have ht :
      ...
      exact Eq.ndrec hraw (complex_add_sub_half z)
    have hpre :
        ∀ᶠ s in 𝓝 ((1 / 2 : ℂ) + z),
    ...
    exact hpre.mono
      (fun s hs => by
        have hcenter : (1 / 2 : ℂ) + (s - (1 / 2 : ℂ)) = s := by
          exact complex_half_add_sub_half s
        have hs_completed :
            completedRiemannZeta ((1 / 2 : ℂ) + (s - (1 / 2 : ℂ))) = 0 := by
          exact hs
        exact hcenter ▸ hs_completed)
