...
example (A : Type u) [CommRing A] :
  ...
  have hdet :
    ...
    have hdet' : (MvPolynomial.map (RingHom.id A)) (hQ.jacobiMatrix.det) = (1 : MvPolynomial (Fin 1) A) := by
      rw [Matrix.det_isEmpty]
    simpa [Algebra.PreSubmersivePresentation.jacobian_eq_jacobiMatrix_det] using hdet'
