...
abbrev StableObj (presentation : Type u) [PresentationQuiver presentation] :=
  FormalPretriangulatedObject presentation

inductive InfObj (presentation : Type u) [PresentationQuiver presentation] : Type u where
...
inductive InfMap {presentation : Type u} [PresentationQuiver presentation] :
    InfObj presentation → InfObj presentation → Type u where
  | base {X Y : StableObj presentation} :
      FormalPretriangulatedMorphism X Y →
...
inductive InfHomotopy {presentation : Type u} [PresentationQuiver presentation] :
...
def infSetoid {presentation : Type u} [PresentationQuiver presentation] (X Y : InfObj presentation) :
    Setoid (InfMap X Y) where
  r := InfHomotopy
  iseqv := ⟨InfHomotopy.refl, fun h => InfHomotopy.symm h,
    fun h₁ h₂ => InfHomotopy.trans h₁ h₂⟩

def Pi0Hom {presentation : Type u} [PresentationQuiver presentation] (X Y : InfObj presentation) : Type u :=
  Quot (infSetoid X Y)

def pi0Class {presentation : Type u} [PresentationQuiver presentation] {X Y : InfObj presentation}
    (f : InfMap X Y) : Pi0Hom X Y :=
  Quot.mk (infSetoid X Y) f

def idPi0 {presentation : Type u} [PresentationQuiver presentation] (X : InfObj presentation) : Pi0Hom X X :=
  pi0Class (InfMap.id X)

def compPi0 {presentation : Type u} [PresentationQuiver presentation] {X Y Z : InfObj presentation}
...
theorem compPi0_id_left {presentation : Type u} [PresentationQuiver presentation]
...
theorem pi0_id_left {presentation : Type u} [PresentationQuiver presentation]
    {X Y : InfObj presentation} (f : Pi0Hom X Y) :
    compPi0 (idPi0 X) f = f :=
  compPi0_id_left f

theorem compPi0_id_right {presentation : Type u} [PresentationQuiver presentation]
    {X Y : InfObj presentation} (f : Pi0Hom X Y) :
...
theorem pi0_id_right {presentation : Type u} [PresentationQuiver presentation]
    {X Y : InfObj presentation} (f : Pi0Hom X Y) :
    compPi0 f (idPi0 Y) = f :=
  compPi0_id_right f

theorem compPi0_assoc {presentation : Type u} [PresentationQuiver presentation]
...
theorem pi0_assoc {presentation : Type u} [PresentationQuiver presentation]
    {W X Y Z : InfObj presentation}
    (f : Pi0Hom W X) (g : Pi0Hom X Y) (h : Pi0Hom Y Z) :
    compPi0 (compPi0 f g) h = compPi0 f (compPi0 g h) :=
  compPi0_assoc f g h

def shiftObj {presentation : Type u} [PresentationQuiver presentation] : InfObj presentation → InfObj presentation :=
  InfObj.shift

def shiftMap {presentation : Type u} [PresentationQuiver presentation] {X Y : InfObj presentation}
    (f : InfMap X Y) : InfMap (shiftObj X) (shiftObj Y) :=
  InfMap.shiftMap f

theorem shift_respects {presentation : Type u} [PresentationQuiver presentation]
    {X Y : InfObj presentation} {f g : InfMap X Y} :
    InfHomotopy f g → InfHomotopy (shiftMap f) (shiftMap g) :=
  InfHomotopy.shift_congr

theorem shift_respects_homotopy {presentation : Type u} [PresentationQuiver presentation]
    {X Y : InfObj presentation} {f g : InfMap X Y} :
    InfHomotopy f g → InfHomotopy (shiftMap f) (shiftMap g) :=
  shift_respects

def shiftMapPi0 {presentation : Type u} [PresentationQuiver presentation] {X Y : InfObj presentation}
...
theorem shift_descends_to_pi0 {presentation : Type u} [PresentationQuiver presentation]
    {X Y : InfObj presentation} {f g : InfMap X Y}
    (h : InfHomotopy f g) :
    shiftMapPi0 (pi0Class f) = shiftMapPi0 (pi0Class g) :=
  Quot.sound (shift_respects_homotopy h)

theorem shiftMapPi0_id {presentation : Type u} [PresentationQuiver presentation] (X : InfObj presentation) :
    shiftMapPi0 (idPi0 X) = idPi0 (shiftObj X) :=
  Quot.sound (InfHomotopy.shift_id X)

theorem shiftMapPi0_comp {presentation : Type u} [PresentationQuiver presentation]
...
def tensorObj {presentation : Type u} [PresentationQuiver presentation] :
    InfObj presentation → InfObj presentation → InfObj presentation :=
  InfObj.tensor

def tensorMap {presentation : Type u} [PresentationQuiver presentation] {A B C D : InfObj presentation}
    (f : InfMap A B) (g : InfMap C D) :
    InfMap (tensorObj A C) (tensorObj B D) :=
  InfMap.tensorMap f g

theorem tensor_respects {presentation : Type u} [PresentationQuiver presentation]
...
theorem tensor_respects_homotopy {presentation : Type u} [PresentationQuiver presentation]
    {A B C D : InfObj presentation}
    {f f' : InfMap A B} {g g' : InfMap C D} :
    InfHomotopy f f' → InfHomotopy g g' →
      InfHomotopy (tensorMap f g) (tensorMap f' g') :=
...
def tensorPi0 {presentation : Type u} [PresentationQuiver presentation] {A B C D : InfObj presentation}
...
theorem tensor_descends_to_pi0 {presentation : Type u} [PresentationQuiver presentation]
    {A B C D : InfObj presentation}
    {f f' : InfMap A B} {g g' : InfMap C D}
...
theorem tensorPi0_id {presentation : Type u} [PresentationQuiver presentation] (A C : InfObj presentation) :
    tensorPi0 (idPi0 A) (idPi0 C) = idPi0 (tensorObj A C) :=
  Quot.sound (InfHomotopy.tensor_id A C)

def cofiberObj {presentation : Type u} [PresentationQuiver presentation] {X Y : InfObj presentation}
    (_f : Pi0Hom X Y) : InfObj presentation :=
  InfObj.cofiber X Y

def fiberObj {presentation : Type u} [PresentationQuiver presentation] {X Y : InfObj presentation}
    (_f : Pi0Hom X Y) : InfObj presentation :=
  InfObj.fiber X Y

inductive DistinguishedTriangle {presentation : Type u} [PresentationQuiver presentation] :
...
def cofiberTriangle {presentation : Type u} [PresentationQuiver presentation] {X Y : InfObj presentation}
    (f : Pi0Hom X Y) : DistinguishedTriangle X Y (cofiberObj f) :=
  DistinguishedTriangle.cofiber f

def fiberTriangle {presentation : Type u} [PresentationQuiver presentation] {X Y : InfObj presentation}
    (f : Pi0Hom X Y) : DistinguishedTriangle (fiberObj f) X Y :=
  DistinguishedTriangle.fiber f

def cofiberTriangleCompatibility {presentation : Type u} [PresentationQuiver presentation]
    {X Y Z : InfObj presentation} (f : Pi0Hom X Y) (g : Pi0Hom Y Z) :
    DistinguishedTriangle (cofiberObj f) (cofiberObj (compPi0 f g)) (cofiberObj g) :=
  DistinguishedTriangle.cofiberComp f g

def triangulatedRotation {presentation : Type u} [PresentationQuiver presentation]
    {X Y Z : InfObj presentation} :
    DistinguishedTriangle X Y Z →
      DistinguishedTriangle Y Z (shiftObj X) :=
  DistinguishedTriangle.rotate

structure SyntacticRealizationData (presentation : Type u) [PresentationQuiver presentation] where
...
private def syntacticIdentityRealizationData (presentation : Type u) [PresentationQuiver presentation] :
...
def realizationMapPi0 {presentation : Type u} [PresentationQuiver presentation]
...
def SyntacticRealizationCompatibilityStatement {presentation : Type u} [PresentationQuiver presentation]
...
private theorem syntacticRealizationCompatibilityWitness {presentation : Type u} [PresentationQuiver presentation]
    (R : SyntacticRealizationData presentation) :
    SyntacticRealizationCompatibilityStatement R := by
  intro X Y f
  rfl

structure SyntacticCompletedPresentationData (presentation : Type u) [PresentationQuiver presentation] where
...
private def syntacticIdentityCompletedPresentationData (presentation : Type u) [PresentationQuiver presentation] :
...
def completedPresentationMapPi0 {presentation : Type u} [PresentationQuiver presentation]
...
def SyntacticCompletedPresentationCompatibilityStatement {presentation : Type u} [PresentationQuiver presentation]
    (P : SyntacticCompletedPresentationData presentation) : Prop :=
  ∀ {X Y : InfObj presentation} (f : InfMap X Y),
    completedPresentationMapPi0 P (pi0Class f) = pi0Class f

private theorem syntacticCompletedPresentationCompatibilityWitness {presentation : Type u} [PresentationQuiver presentation]
    (P : SyntacticCompletedPresentationData presentation) :
    SyntacticCompletedPresentationCompatibilityStatement P := by
  intro X Y f
  exact Quot.sound (P.generator_agrees f)

private def syntacticInfinityCategoryTarget (presentation : Type u) [PresentationQuiver presentation] :
    InfinityCategoryTarget (InfObj presentation) where
  Mapping := InfMap
  Pi0Hom := Pi0Hom
  pi0Class := fun {X Y} f => pi0Class (presentation := presentation) f
  idPi0 := fun X => idPi0 (presentation := presentation) X
  compPi0 := fun f g => compPi0 (presentation := presentation) f g

private def syntacticInfinityShiftTarget (presentation : Type u) [PresentationQuiver presentation] :
  InfinityShiftTarget (syntacticInfinityCategoryTarget presentation) where
  shiftObj := shiftObj
  shiftMapPi0 := fun {X Y} f => shiftMapPi0 (presentation := presentation) f

private def syntacticInfinityTriangulatedTarget (presentation : Type u) [PresentationQuiver presentation] :
    InfinityTriangulatedTarget
  (syntacticInfinityCategoryTarget presentation)
  (syntacticInfinityShiftTarget presentation) where
  cofiberObj := fun {X Y} f => cofiberObj (presentation := presentation) f
  fiberObj := fun {X Y} f => fiberObj (presentation := presentation) f
  distinguishedTriangle := DistinguishedTriangle

private def syntacticInfinityMonoidalTarget (presentation : Type u) [PresentationQuiver presentation] :
  InfinityMonoidalTarget (syntacticInfinityCategoryTarget presentation) where
  tensorObj := tensorObj
  tensorPi0 := fun {A B C D} f g => tensorPi0 (presentation := presentation) f g

private def syntacticInfinityRealizationTarget (presentation : Type u) [PresentationQuiver presentation] :
    InfinityRealizationTarget
      (syntacticInfinityCategoryTarget presentation)
      (syntacticInfinityShiftTarget presentation)
      (syntacticInfinityTriangulatedTarget presentation)
      (syntacticInfinityMonoidalTarget presentation) where
  targetObj := InfObj presentation
  targetCategory := syntacticInfinityCategoryTarget presentation
  targetShift := syntacticInfinityShiftTarget presentation
  targetTriangulated := syntacticInfinityTriangulatedTarget presentation
  targetMonoidal := syntacticInfinityMonoidalTarget presentation
  objectRealization := (syntacticIdentityRealizationData presentation).mapObj
  pi0Realization := realizationMapPi0 (syntacticIdentityRealizationData presentation)

private def syntacticCompletedPresentationTarget (presentation : Type u)
  [PresentationQuiver presentation] :
    InfinityCompletedPresentationTarget
      ...
      (syntacticInfinityTriangulatedTarget presentation)
      (syntacticInfinityMonoidalTarget presentation) where
  mapPi0 := completedPresentationMapPi0 (syntacticIdentityCompletedPresentationData presentation)
...
def syntactic (presentation : Type u)
  ...
  stableCompletion := StableCompletionConstructionTarget.syntactic presentation
  Obj := InfObj presentation
  category := syntacticInfinityCategoryTarget presentation
  shift := syntacticInfinityShiftTarget presentation
  triangulated := syntacticInfinityTriangulatedTarget presentation
  monoidal := syntacticInfinityMonoidalTarget presentation
  realization := syntacticInfinityRealizationTarget presentation
  completedPresentation := syntacticCompletedPresentationTarget presentation
...
def syntactic (presentation : Type u)
  [PresentationQuiver presentation] :
    StableInfinityEnhancementData
      (StableInfinityEnhancementTarget.syntactic presentation) where
  categoryData :=
    { categoryLaws :=
        { idComp := by
            intro X Y f
            simpa [StableInfinityEnhancementTarget.syntactic, syntacticInfinityCategoryTarget] using
              (compPi0_id_left (presentation := presentation) f)
          compId := by
            intro X Y f
            simpa [StableInfinityEnhancementTarget.syntactic, syntacticInfinityCategoryTarget] using
              (compPi0_id_right (presentation := presentation) f)
          assoc := by
            intro W X Y Z f g h
            simpa [StableInfinityEnhancementTarget.syntactic, syntacticInfinityCategoryTarget] using
              (compPi0_assoc (presentation := presentation) f g h) } }
  shiftData :=
    { shiftCompatibility :=
        { idCompatibility := by
            intro X
            simpa [StableInfinityEnhancementTarget.syntactic, syntacticInfinityCategoryTarget,
              syntacticInfinityShiftTarget] using
              (shiftMapPi0_id (presentation := presentation) X)
          compCompatibility := by
            intro X Y Z f g
            simpa [StableInfinityEnhancementTarget.syntactic, syntacticInfinityCategoryTarget,
              syntacticInfinityShiftTarget] using
              (shiftMapPi0_comp (presentation := presentation) f g) } }
  triangulatedData :=
    { cofiberTriangleWitness := by
        intro X Y f
        simpa [StableInfinityEnhancementTarget.syntactic, syntacticInfinityCategoryTarget,
          syntacticInfinityShiftTarget, syntacticInfinityTriangulatedTarget] using
          (cofiberTriangle (presentation := presentation) f)
      fiberTriangleWitness := by
        intro X Y f
        simpa [StableInfinityEnhancementTarget.syntactic, syntacticInfinityCategoryTarget,
          syntacticInfinityShiftTarget, syntacticInfinityTriangulatedTarget] using
          (fiberTriangle (presentation := presentation) f)
      cofiberCompositionWitness := by
        intro X Y Z f g
        simpa [StableInfinityEnhancementTarget.syntactic, syntacticInfinityCategoryTarget,
          syntacticInfinityShiftTarget, syntacticInfinityTriangulatedTarget] using
          (cofiberTriangleCompatibility (presentation := presentation) f g)
      rotationWitness := by
        intro X Y Z triangle
        simpa [StableInfinityEnhancementTarget.syntactic, syntacticInfinityCategoryTarget,
          syntacticInfinityShiftTarget, syntacticInfinityTriangulatedTarget] using
          (triangulatedRotation (presentation := presentation) triangle) }
  monoidalData :=
    { monoidalCompatibility :=
        { tensorIdCompatibility := by
            intro A C
            simpa [StableInfinityEnhancementTarget.syntactic, syntacticInfinityCategoryTarget,
              syntacticInfinityMonoidalTarget] using
              (tensorPi0_id (presentation := presentation) A C) } }
  realizationData :=
    { compatibility :=
        { identityCompatibility := by
            intro X
            simpa [StableInfinityEnhancementTarget.syntactic, syntacticInfinityCategoryTarget,
              syntacticInfinityShiftTarget, syntacticInfinityTriangulatedTarget,
              syntacticInfinityMonoidalTarget, syntacticInfinityRealizationTarget,
              realizationMapPi0] using
              (Quot.sound (InfHomotopy.realization_compat (InfMap.id X)))
          compositionCompatibility := by
            ...
            simpa [StableInfinityEnhancementTarget.syntactic, syntacticInfinityCategoryTarget,
              syntacticInfinityShiftTarget, syntacticInfinityTriangulatedTarget,
              syntacticInfinityMonoidalTarget, syntacticInfinityRealizationTarget,
              realizationMapPi0, compPi0] using
              (Quot.sound <|
                InfHomotopy.trans
          ...
          triangulatedCompatibility := by
            ...
            simpa [StableInfinityEnhancementTarget.syntactic, syntacticInfinityCategoryTarget,
              syntacticInfinityShiftTarget, syntacticInfinityTriangulatedTarget,
              syntacticInfinityMonoidalTarget, syntacticInfinityRealizationTarget] using
              (cofiberTriangle (presentation := presentation)
                (realizationMapPi0 (presentation := presentation)
                  (syntacticIdentityRealizationData presentation) f))
  ...
  completedPresentationData :=
    { compatibility :=
        { compatibilityWithCompletedPresentation := by
            ...
            simpa [StableInfinityEnhancementTarget.syntactic, syntacticInfinityCategoryTarget,
              syntacticInfinityShiftTarget, syntacticInfinityTriangulatedTarget,
              syntacticInfinityMonoidalTarget, syntacticCompletedPresentationTarget,
              completedPresentationMapPi0] using
              (syntacticCompletedPresentationCompatibilityWitness (presentation := presentation)
                (syntacticIdentityCompletedPresentationData presentation) f) } }
