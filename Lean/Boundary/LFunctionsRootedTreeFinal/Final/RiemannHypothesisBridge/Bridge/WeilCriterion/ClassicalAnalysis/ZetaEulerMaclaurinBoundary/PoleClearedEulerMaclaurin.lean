...
theorem Complex.poleClearedRiemannZeta_boundaryLine_growth_bound_of_zeta_log :
      ...
      ∀ t : ℝ,
        1 ≤ ‖t‖ →
          ‖((Complex.boundaryLineOnePointRealParam t - 1) *
            ...
            A * (1 + ‖t‖) ^ m := by
  rcases Complex.riemannZeta_boundaryLine_log_bound with
    ⟨A, hA_pos, hzeta⟩
  refine ⟨2 * A, 2, mul_pos two_pos hA_pos, ?_⟩
  intro t ht
  let H : ℝ := 1 + ‖t‖
  have hH_nonneg : 0 ≤ H :=
  ...
  have hzeta_rhs_nonneg :
      0 ≤ A * Real.log (2 + ‖t‖) :=
    le_trans (norm_nonneg (riemannZeta (Complex.boundaryLineOnePointRealParam t)))
      hzeta_bound
