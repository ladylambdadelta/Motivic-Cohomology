  ...
  have hinf :
    ...
    exact
      Complex.finiteAbelPlana_log_integerResidueIsolationRadius_pos
        hw n
  dsimp [Complex.finiteAbelPlanaLogPuncturedRectangleRadiusBound]
  exact lt_min hquarter (lt_min hheight hinf)
...
theorem Complex.finiteAbelPlana_log_puncturedRectangle_radius_lt_quarter
    ...
    ρ < (1 : ℝ) / 4 := by
  dsimp [Complex.finiteAbelPlanaLogPuncturedRectangleRadiusBound] at hρR
  exact lt_of_lt_of_le hρR
    (min_le_left ((1 : ℝ) / 4)
      (min (|T| / 2)
        (Finset.inf' (Finset.range (N + 2))
          ⟨0, by simp⟩
  ...
  have hρ_inf :
      ρ <
        Finset.inf' (Finset.range (N + 2))
          ⟨0, by simp⟩
          (fun n => Complex.finiteAbelPlanaLogIntegerResidueIsolationRadius w n) :=
    lt_of_lt_of_le hρR hbound_le_inf
  exact (Finset.lt_inf'_iff.mp hρ_inf) n hn
...
theorem Complex.mem_finiteAbelPlanaPuncturedRectangle_iff
  ...
  dsimp [Complex.finiteAbelPlanaPuncturedRectangle]
  constructor
  · intro hz
    refine ⟨hz.1, ?_⟩
    intro n hn hball
    exact hz.2 (Set.mem_iUnion.2 ⟨n, Set.mem_iUnion.2 ⟨hn, hball⟩⟩)
  · intro hz
    refine ⟨hz.1, ?_⟩
    intro hmem
    rcases Set.mem_iUnion.1 hmem with ⟨n, hnmem⟩
    rcases Set.mem_iUnion.1 hnmem with ⟨hn, hball⟩
    exact hz.2 n hn hball
...
theorem Complex.finiteAbelPlana_log_sin_pi_mul_ne_zero_of_mem_puncturedRectangle
  ...
  rcases
    Complex.finiteAbelPlana_integerPole_of_sin_pi_mul_eq_zero_of_mem_closedRectangle
      hzdata.1 hzero with ⟨n, hn, hz_eq⟩
