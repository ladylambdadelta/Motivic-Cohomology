...
This is the single standard complex Stirling primitive used by the
completed-normalization finite-order wrappers below.  The exclusion `1 ≤ ‖z‖` is
necessary for the classical right-half-plane Stirling region.  In Mathlib's finite-valued
`Gammaℝ`, the classical pole at `0` is represented by the zero `Gammaℝ 0 = 0`; on the stated
region the factor is nonzero by `Gammaℝ_ne_zero_of_re_nonneg_and_one_le_norm`. -/
