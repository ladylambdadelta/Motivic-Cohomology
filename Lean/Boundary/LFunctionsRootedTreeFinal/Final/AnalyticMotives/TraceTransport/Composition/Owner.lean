...
/-- The raw identity trace transport on a presentation spine. -/
def TraceTransport.id
    (object : TraceTransportObject) : TraceTransport :=
  (object, object, TraceRewritePath.id object.source)
...
def TraceTransport.comp
  ...
  (first.source, second.target, first.path.comp second.path)
